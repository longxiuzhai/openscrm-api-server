...
CREATE TABLE `customer_staff_relation_history` (
  ...
  `ext_customer_id` char(64) COMMENT '客户ID',
  `createtime` datetime(3) COMMENT '员工添加客户的时间',
  `customer_delete_staff_at` datetime(3) COMMENT '客户删除员工的时间',
  `staff_delete_customer_at` datetime(3) COMMENT '员工删除客户的时间',
  `created_at` datetime(3) COMMENT '创建时间',
  `updated_at` datetime(3) COMMENT '更新时间',
  `deleted_at` datetime(3) COMMENT '删除时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE `group_chat_tags` (
  ...
  `group_chat_tag_id` bigint NOT NULL,
  PRIMARY KEY (`group_chat_id`, `group_chat_tag_id`),
  KEY `idx_group_chat_tag_id` (`group_chat_tag_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='客户群-标签关联';
